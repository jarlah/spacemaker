import { RootSchema } from "./schema-types";
import Ajv, { ErrorObject } from "ajv";

const ajv = new Ajv();

/**
 * Get validated payload or error
 *
 * @param event
 * @returns
 */
export function validateRequestBody(
  body: string | null
):
  | { success: true; data: RootSchema }
  | { success: false; data?: ErrorObject[] | null } {
  const data: RootSchema = JSON.parse(body || "{}");

  // Validate json
  if (!ajv.validate(schema, data)) {
    return {
      success: false,
      data: ajv.errors,
    };
  }

  return { success: true, data };
}

// TODO add constraints on coordinates for longitude (-180 to 180) and latitude (-90 to 90)
// This schema is unoptimized, and should be refactored into smaller maintainable pieces
const schema = {
  type: "object",
  default: {},
  title: "Root Schema",
  required: ["building_limits", "height_plateaus"],
  properties: {
    building_limits: {
      type: "object",
      default: {},
      title: "The building_limits Schema",
      required: ["type", "features"],
      properties: {
        type: {
          type: "string",
          default: "",
          title: "The type Schema",
          examples: ["FeatureCollection"],
        },
        features: {
          type: "array",
          default: [],
          title: "The features Schema",
          items: {
            type: "object",
            default: {},
            title: "A Schema",
            required: ["type", "properties", "geometry"],
            properties: {
              type: {
                type: "string",
                default: "",
                title: "The type Schema",
                examples: ["Feature"],
              },
              properties: {
                type: "object",
                default: {},
                title: "The properties Schema",
                required: [],
                properties: {},
                examples: [{}],
              },
              geometry: {
                type: "object",
                default: {},
                title: "The geometry Schema",
                required: ["type", "coordinates"],
                properties: {
                  type: {
                    type: "string",
                    default: "",
                    title: "The type Schema",
                    examples: ["Polygon"],
                  },
                  coordinates: {
                    type: "array",
                    default: [],
                    title: "The coordinates Schema",
                    items: {
                      type: "array",
                      default: [],
                      title: "A Schema",
                      items: {
                        type: "array",
                        title: "A Schema",
                        items: {
                          type: "number",
                          title: "A Schema",
                          examples: [
                            10.757867266534337, 59.91339283457274,
                            10.756516000002959, 59.913633000004204,
                            10.756398999995643, 59.91346700000333,
                            10.75628300000438, 59.91330300000502,
                            10.756052815307351, 59.91297582153187,
                            10.756245682709302, 59.912959479672516,
                            10.757486364709461, 59.91285434826322,
                          ],
                        },
                        examples: [
                          [10.757867266534337, 59.91339283457274],
                          [10.756516000002959, 59.913633000004204],
                          [10.756398999995643, 59.91346700000333],
                          [10.75628300000438, 59.91330300000502],
                          [10.756052815307351, 59.91297582153187],
                          [10.756245682709302, 59.912959479672516],
                          [10.757486364709461, 59.91285434826322],
                          [10.757867266534337, 59.91339283457274],
                        ],
                      },
                      examples: [
                        [
                          [10.757867266534337, 59.91339283457274],
                          [10.756516000002959, 59.913633000004204],
                          [10.756398999995643, 59.91346700000333],
                          [10.75628300000438, 59.91330300000502],
                          [10.756052815307351, 59.91297582153187],
                          [10.756245682709302, 59.912959479672516],
                          [10.757486364709461, 59.91285434826322],
                          [10.757867266534337, 59.91339283457274],
                        ],
                      ],
                    },
                    examples: [
                      [
                        [
                          [10.757867266534337, 59.91339283457274],
                          [10.756516000002959, 59.913633000004204],
                          [10.756398999995643, 59.91346700000333],
                          [10.75628300000438, 59.91330300000502],
                          [10.756052815307351, 59.91297582153187],
                          [10.756245682709302, 59.912959479672516],
                          [10.757486364709461, 59.91285434826322],
                          [10.757867266534337, 59.91339283457274],
                        ],
                      ],
                    ],
                  },
                },
                examples: [
                  {
                    type: "Polygon",
                    coordinates: [
                      [
                        [10.757867266534337, 59.91339283457274],
                        [10.756516000002959, 59.913633000004204],
                        [10.756398999995643, 59.91346700000333],
                        [10.75628300000438, 59.91330300000502],
                        [10.756052815307351, 59.91297582153187],
                        [10.756245682709302, 59.912959479672516],
                        [10.757486364709461, 59.91285434826322],
                        [10.757867266534337, 59.91339283457274],
                      ],
                    ],
                  },
                ],
              },
            },
            examples: [
              {
                type: "Feature",
                properties: {},
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.757867266534337, 59.91339283457274],
                      [10.756516000002959, 59.913633000004204],
                      [10.756398999995643, 59.91346700000333],
                      [10.75628300000438, 59.91330300000502],
                      [10.756052815307351, 59.91297582153187],
                      [10.756245682709302, 59.912959479672516],
                      [10.757486364709461, 59.91285434826322],
                      [10.757867266534337, 59.91339283457274],
                    ],
                  ],
                },
              },
            ],
          },
          examples: [
            [
              {
                type: "Feature",
                properties: {},
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.757867266534337, 59.91339283457274],
                      [10.756516000002959, 59.913633000004204],
                      [10.756398999995643, 59.91346700000333],
                      [10.75628300000438, 59.91330300000502],
                      [10.756052815307351, 59.91297582153187],
                      [10.756245682709302, 59.912959479672516],
                      [10.757486364709461, 59.91285434826322],
                      [10.757867266534337, 59.91339283457274],
                    ],
                  ],
                },
              },
            ],
          ],
        },
      },
      examples: [
        {
          type: "FeatureCollection",
          features: [
            {
              type: "Feature",
              properties: {},
              geometry: {
                type: "Polygon",
                coordinates: [
                  [
                    [10.757867266534337, 59.91339283457274],
                    [10.756516000002959, 59.913633000004204],
                    [10.756398999995643, 59.91346700000333],
                    [10.75628300000438, 59.91330300000502],
                    [10.756052815307351, 59.91297582153187],
                    [10.756245682709302, 59.912959479672516],
                    [10.757486364709461, 59.91285434826322],
                    [10.757867266534337, 59.91339283457274],
                  ],
                ],
              },
            },
          ],
        },
      ],
    },
    height_plateaus: {
      type: "object",
      default: {},
      title: "The height_plateaus Schema",
      required: ["type", "features"],
      properties: {
        type: {
          type: "string",
          default: "",
          title: "The type Schema",
          examples: ["FeatureCollection"],
        },
        features: {
          type: "array",
          default: [],
          title: "The features Schema",
          items: {
            type: "object",
            title: "A Schema",
            required: ["type", "geometry", "properties"],
            properties: {
              type: {
                type: "string",
                title: "The type Schema",
                examples: ["Feature"],
              },
              geometry: {
                type: "object",
                title: "The geometry Schema",
                required: ["type", "coordinates"],
                properties: {
                  type: {
                    type: "string",
                    title: "The type Schema",
                    examples: ["Polygon"],
                  },
                  coordinates: {
                    type: "array",
                    title: "The coordinates Schema",
                    items: {
                      type: "array",
                      title: "A Schema",
                      items: {
                        type: "array",
                        title: "A Schema",
                        items: {
                          type: "number",
                          title: "A Schema",
                          examples: [
                            10.75678086443506, 59.91291413160555,
                            10.757486364709461, 59.91285434826322,
                            10.757867266534337, 59.91339283457274,
                            10.757212164399775, 59.91350927037677,
                            10.756996990155885, 59.91321236033006,
                            10.756516000002959, 59.913633000004204,
                            10.756398999995643, 59.91346700000333,
                            10.756312148500106, 59.91334421011477,
                            10.75628300000438, 59.91330300000502,
                            10.756052815307351, 59.91297582153187,
                            10.756245682709302, 59.912959479672516,
                          ],
                        },
                        examples: [
                          [10.75678086443506, 59.91291413160555],
                          [10.757486364709461, 59.91285434826322],
                          [10.757867266534337, 59.91339283457274],
                          [10.757212164399775, 59.91350927037677],
                          [10.75678086443506, 59.91291413160555],
                          [10.756996990155885, 59.91321236033006],
                          [10.757212164399775, 59.91350927037677],
                          [10.756516000002959, 59.913633000004204],
                          [10.756398999995643, 59.91346700000333],
                          [10.756312148500106, 59.91334421011477],
                          [10.756996990155885, 59.91321236033006],
                          [10.756312148500106, 59.91334421011477],
                          [10.75628300000438, 59.91330300000502],
                          [10.756052815307351, 59.91297582153187],
                          [10.756245682709302, 59.912959479672516],
                          [10.75678086443506, 59.91291413160555],
                          [10.756996990155885, 59.91321236033006],
                          [10.756312148500106, 59.91334421011477],
                        ],
                      },
                      examples: [
                        [
                          [10.75678086443506, 59.91291413160555],
                          [10.757486364709461, 59.91285434826322],
                          [10.757867266534337, 59.91339283457274],
                          [10.757212164399775, 59.91350927037677],
                          [10.75678086443506, 59.91291413160555],
                        ],
                        [
                          [10.756996990155885, 59.91321236033006],
                          [10.757212164399775, 59.91350927037677],
                          [10.756516000002959, 59.913633000004204],
                          [10.756398999995643, 59.91346700000333],
                          [10.756312148500106, 59.91334421011477],
                          [10.756996990155885, 59.91321236033006],
                        ],
                        [
                          [10.756312148500106, 59.91334421011477],
                          [10.75628300000438, 59.91330300000502],
                          [10.756052815307351, 59.91297582153187],
                          [10.756245682709302, 59.912959479672516],
                          [10.75678086443506, 59.91291413160555],
                          [10.756996990155885, 59.91321236033006],
                          [10.756312148500106, 59.91334421011477],
                        ],
                      ],
                    },
                    examples: [
                      [
                        [
                          [10.75678086443506, 59.91291413160555],
                          [10.757486364709461, 59.91285434826322],
                          [10.757867266534337, 59.91339283457274],
                          [10.757212164399775, 59.91350927037677],
                          [10.75678086443506, 59.91291413160555],
                        ],
                      ],
                      [
                        [
                          [10.756996990155885, 59.91321236033006],
                          [10.757212164399775, 59.91350927037677],
                          [10.756516000002959, 59.913633000004204],
                          [10.756398999995643, 59.91346700000333],
                          [10.756312148500106, 59.91334421011477],
                          [10.756996990155885, 59.91321236033006],
                        ],
                      ],
                      [
                        [
                          [10.756312148500106, 59.91334421011477],
                          [10.75628300000438, 59.91330300000502],
                          [10.756052815307351, 59.91297582153187],
                          [10.756245682709302, 59.912959479672516],
                          [10.75678086443506, 59.91291413160555],
                          [10.756996990155885, 59.91321236033006],
                          [10.756312148500106, 59.91334421011477],
                        ],
                      ],
                    ],
                  },
                },
                examples: [
                  {
                    type: "Polygon",
                    coordinates: [
                      [
                        [10.75678086443506, 59.91291413160555],
                        [10.757486364709461, 59.91285434826322],
                        [10.757867266534337, 59.91339283457274],
                        [10.757212164399775, 59.91350927037677],
                        [10.75678086443506, 59.91291413160555],
                      ],
                    ],
                  },
                  {
                    type: "Polygon",
                    coordinates: [
                      [
                        [10.756996990155885, 59.91321236033006],
                        [10.757212164399775, 59.91350927037677],
                        [10.756516000002959, 59.913633000004204],
                        [10.756398999995643, 59.91346700000333],
                        [10.756312148500106, 59.91334421011477],
                        [10.756996990155885, 59.91321236033006],
                      ],
                    ],
                  },
                  {
                    type: "Polygon",
                    coordinates: [
                      [
                        [10.756312148500106, 59.91334421011477],
                        [10.75628300000438, 59.91330300000502],
                        [10.756052815307351, 59.91297582153187],
                        [10.756245682709302, 59.912959479672516],
                        [10.75678086443506, 59.91291413160555],
                        [10.756996990155885, 59.91321236033006],
                        [10.756312148500106, 59.91334421011477],
                      ],
                    ],
                  },
                ],
              },
              properties: {
                type: "object",
                title: "The properties Schema",
                required: ["elevation"],
                properties: {
                  elevation: {
                    type: "number",
                    title: "The elevation Schema",
                    examples: [3.63, 4.63, 2.63],
                  },
                },
                examples: [
                  {
                    elevation: 3.63,
                  },
                  {
                    elevation: 4.63,
                  },
                  {
                    elevation: 2.63,
                  },
                ],
              },
            },
            examples: [
              {
                type: "Feature",
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.75678086443506, 59.91291413160555],
                      [10.757486364709461, 59.91285434826322],
                      [10.757867266534337, 59.91339283457274],
                      [10.757212164399775, 59.91350927037677],
                      [10.75678086443506, 59.91291413160555],
                    ],
                  ],
                },
                properties: {
                  elevation: 3.63,
                },
              },
              {
                type: "Feature",
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.756996990155885, 59.91321236033006],
                      [10.757212164399775, 59.91350927037677],
                      [10.756516000002959, 59.913633000004204],
                      [10.756398999995643, 59.91346700000333],
                      [10.756312148500106, 59.91334421011477],
                      [10.756996990155885, 59.91321236033006],
                    ],
                  ],
                },
                properties: {
                  elevation: 4.63,
                },
              },
              {
                type: "Feature",
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.756312148500106, 59.91334421011477],
                      [10.75628300000438, 59.91330300000502],
                      [10.756052815307351, 59.91297582153187],
                      [10.756245682709302, 59.912959479672516],
                      [10.75678086443506, 59.91291413160555],
                      [10.756996990155885, 59.91321236033006],
                      [10.756312148500106, 59.91334421011477],
                    ],
                  ],
                },
                properties: {
                  elevation: 2.63,
                },
              },
            ],
          },
          examples: [
            [
              {
                type: "Feature",
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.75678086443506, 59.91291413160555],
                      [10.757486364709461, 59.91285434826322],
                      [10.757867266534337, 59.91339283457274],
                      [10.757212164399775, 59.91350927037677],
                      [10.75678086443506, 59.91291413160555],
                    ],
                  ],
                },
                properties: {
                  elevation: 3.63,
                },
              },
              {
                type: "Feature",
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.756996990155885, 59.91321236033006],
                      [10.757212164399775, 59.91350927037677],
                      [10.756516000002959, 59.913633000004204],
                      [10.756398999995643, 59.91346700000333],
                      [10.756312148500106, 59.91334421011477],
                      [10.756996990155885, 59.91321236033006],
                    ],
                  ],
                },
                properties: {
                  elevation: 4.63,
                },
              },
              {
                type: "Feature",
                geometry: {
                  type: "Polygon",
                  coordinates: [
                    [
                      [10.756312148500106, 59.91334421011477],
                      [10.75628300000438, 59.91330300000502],
                      [10.756052815307351, 59.91297582153187],
                      [10.756245682709302, 59.912959479672516],
                      [10.75678086443506, 59.91291413160555],
                      [10.756996990155885, 59.91321236033006],
                      [10.756312148500106, 59.91334421011477],
                    ],
                  ],
                },
                properties: {
                  elevation: 2.63,
                },
              },
            ],
          ],
        },
      },
      examples: [
        {
          type: "FeatureCollection",
          features: [
            {
              type: "Feature",
              geometry: {
                type: "Polygon",
                coordinates: [
                  [
                    [10.75678086443506, 59.91291413160555],
                    [10.757486364709461, 59.91285434826322],
                    [10.757867266534337, 59.91339283457274],
                    [10.757212164399775, 59.91350927037677],
                    [10.75678086443506, 59.91291413160555],
                  ],
                ],
              },
              properties: {
                elevation: 3.63,
              },
            },
            {
              type: "Feature",
              geometry: {
                type: "Polygon",
                coordinates: [
                  [
                    [10.756996990155885, 59.91321236033006],
                    [10.757212164399775, 59.91350927037677],
                    [10.756516000002959, 59.913633000004204],
                    [10.756398999995643, 59.91346700000333],
                    [10.756312148500106, 59.91334421011477],
                    [10.756996990155885, 59.91321236033006],
                  ],
                ],
              },
              properties: {
                elevation: 4.63,
              },
            },
            {
              type: "Feature",
              geometry: {
                type: "Polygon",
                coordinates: [
                  [
                    [10.756312148500106, 59.91334421011477],
                    [10.75628300000438, 59.91330300000502],
                    [10.756052815307351, 59.91297582153187],
                    [10.756245682709302, 59.912959479672516],
                    [10.75678086443506, 59.91291413160555],
                    [10.756996990155885, 59.91321236033006],
                    [10.756312148500106, 59.91334421011477],
                  ],
                ],
              },
              properties: {
                elevation: 2.63,
              },
            },
          ],
        },
      ],
    },
  },
  examples: [
    {
      building_limits: {
        type: "FeatureCollection",
        features: [
          {
            type: "Feature",
            properties: {},
            geometry: {
              type: "Polygon",
              coordinates: [
                [
                  [10.757867266534337, 59.91339283457274],
                  [10.756516000002959, 59.913633000004204],
                  [10.756398999995643, 59.91346700000333],
                  [10.75628300000438, 59.91330300000502],
                  [10.756052815307351, 59.91297582153187],
                  [10.756245682709302, 59.912959479672516],
                  [10.757486364709461, 59.91285434826322],
                  [10.757867266534337, 59.91339283457274],
                ],
              ],
            },
          },
        ],
      },
      height_plateaus: {
        type: "FeatureCollection",
        features: [
          {
            type: "Feature",
            geometry: {
              type: "Polygon",
              coordinates: [
                [
                  [10.75678086443506, 59.91291413160555],
                  [10.757486364709461, 59.91285434826322],
                  [10.757867266534337, 59.91339283457274],
                  [10.757212164399775, 59.91350927037677],
                  [10.75678086443506, 59.91291413160555],
                ],
              ],
            },
            properties: {
              elevation: 3.63,
            },
          },
          {
            type: "Feature",
            geometry: {
              type: "Polygon",
              coordinates: [
                [
                  [10.756996990155885, 59.91321236033006],
                  [10.757212164399775, 59.91350927037677],
                  [10.756516000002959, 59.913633000004204],
                  [10.756398999995643, 59.91346700000333],
                  [10.756312148500106, 59.91334421011477],
                  [10.756996990155885, 59.91321236033006],
                ],
              ],
            },
            properties: {
              elevation: 4.63,
            },
          },
          {
            type: "Feature",
            geometry: {
              type: "Polygon",
              coordinates: [
                [
                  [10.756312148500106, 59.91334421011477],
                  [10.75628300000438, 59.91330300000502],
                  [10.756052815307351, 59.91297582153187],
                  [10.756245682709302, 59.912959479672516],
                  [10.75678086443506, 59.91291413160555],
                  [10.756996990155885, 59.91321236033006],
                  [10.756312148500106, 59.91334421011477],
                ],
              ],
            },
            properties: {
              elevation: 2.63,
            },
          },
        ],
      },
    },
  ],
};

export default schema;
